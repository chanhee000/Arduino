...
void check_maze_status(void)
{
  ...
  else if ((left_sonar >= 0) && (left_sonar <=250) && (front_sonar >= 0) && (front_sonar <=130))
  {
    maze_status = 2;
    Serial.println("maze_status = 2");

  }
  else if ((right_sonar >= 0) && (right_sonar <=220) && (front_sonar >= 0) && (front_sonar <=130))
  {
    maze_status = 3;
...
void wall_collision_avoid(int base_speed)
{
  ...
  right_pwm = (base_speed - error)*0.98;
  left_pwm = (base_speed + error)*1.12;
